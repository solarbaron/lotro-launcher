...
public:
    WelcomePage(QWidget* parent = nullptr) : QWizardPage(parent) {
        ...
        setSubTitle("This wizard will help you set up the launcher for the first time.");
        
        QVBoxLayout* layout = new QVBoxLayout(this);
        
        QLabel* welcomeText = new QLabel(
            "LOTRO Launcher is a cross-platform launcher for Lord of the Rings Online "
            "that provides:\n\n"
...
public:
    GameSelectionPage(SetupWizard::Impl* impl, QWidget* parent = nullptr) 
        ...
        QRadioButton* lotroPreviewBtn = new QRadioButton("LOTRO Preview (Bullroarer)");
        ...
        connect(gameGroup, QOverload<int>::of(&QButtonGroup::idClicked), [this](int id) {
            switch (id) {
                case 0: m_impl->gameId = "lotro"; break;
                case 1: m_impl->gameId = "lotro-preview"; break;
                case 2: m_impl->gameId = "ddo"; break;
            }
        });
...
public:
    GamePathPage(SetupWizard::Impl* impl, QWidget* parent = nullptr) 
        ...
        layout->addWidget(new QLabel("Settings/Addons Directory:"));
...
public:
    WineSetupPage(SetupWizard::Impl* impl, QWidget* parent = nullptr) 
        ...
        QLabel* infoLabel = new QLabel(
            ...
            "download and configure Wine for you, or you can use your own installation."
        );
        infoLabel->setWordWrap(true);
        layout->addWidget(infoLabel);
    ...
    bool validatePage() override {
        ...
        if (!wineManager.isSetup()) {
            ...
            bool success = wineManager.setup([this](size_t current, size_t total) {
                int percent = total > 0 ? static_cast<int>((current * 100) / total) : 0;
                m_impl->wineProgress->setValue(percent);
            });
            
            if (!success) {
                m_impl->wineStatusLabel->setText("Wine setup failed. Check logs for details.");
                return false;
            }
...
public:
    CompletePage(SetupWizard::Impl* impl, QWidget* parent = nullptr) 
        ...
        QLabel* completeLabel = new QLabel(
            "Setup is complete! You can now use LOTRO Launcher to:\n\n"
    ...
    void initializePage() override {
        ...
        gameConfig.gameType = m_impl->gameId.contains("ddo") ? GameType::DDO : GameType::LOTRO;
        gameConfig.gameDirectory = m_impl->gamePath.toStdString();
        
        // On Linux, use the Wine prefix settings path for addons
        // This ensures plugins are installed where LOTRO (running under Wine) can find them
#ifdef PLATFORM_LINUX
        auto& wineManager = WineManager::instance();
        if (wineManager.isSetup()) {
            auto wineSettingsPath = wineManager.getWineLotroSettingsPath();
            gameConfig.settingsDirectory = wineSettingsPath;
            spdlog::info("Using Wine prefix settings path: {}", wineSettingsPath.string());
        } else {
...
QString SetupWizard::gameId() const {
    return m_impl->gameId;
}

QString SetupWizard::gamePath() const {
    return m_impl->gamePath;
}
