...
std::filesystem::path
WineManager::detectSteamPrefix(const std::filesystem::path &gameDirectory,
                               uint32_t appId) {
  // Known Steam App IDs
  ...
  if (appId == 0) {
    spdlog::debug("Could not determine Steam App ID for: {}",
  ...
  if (steamappsPath.empty()) {
    spdlog::debug("Could not find steamapps directory from: {}",
                  gameDirectory.string());
    return {};
  }

  // Check for Proton prefix
  std::filesystem::path compatdataPath =
      steamappsPath / "compatdata" / std::to_string(appId);
...
bool WineManager::setup(DownloadProgressCallback progress,
  ...
  if (m_config.prefixMode == WinePrefixMode::Builtin) {
    ...
    if (statusCb)
      statusCb("Checking for umu-run...");
    if (!isUmuAvailable()) {
      spdlog::error("umu-launcher is not installed. Please install it first.");
      spdlog::error(
          "See: https://github.com/Open-Wine-Components/umu-launcher");
      if (statusCb)
        statusCb("Error: umu-launcher is not installed.");
      return false;
    }
    if (statusCb)
      statusCb("Found umu-run.");

    // Initialize prefix if needed
    if (!isPrefixValid()) {
      spdlog::info("Initializing Wine prefix...");
      if (statusCb)
        statusCb("Initializing Wine prefix...");
      if (!initializePrefix(statusCb)) {
        spdlog::error("Failed to initialize Wine prefix");
        if (statusCb)
          statusCb("Wine prefix initialization failed.");
        return false;
      }
    } else {
      if (statusCb)
        statusCb("Wine prefix already exists.");
    }

    // Set up DXVK if enabled (umu-run usually handles this, but we can force
    // it)
    if (m_config.dxvkEnabled && !isDxvkInstalled()) {
      spdlog::info("Setting up DXVK...");
      if (statusCb)
        statusCb("Setting up DXVK...");
      if (!setupDxvk(progress)) {
        spdlog::warn("Failed to set up DXVK, continuing without it");
        if (statusCb)
          statusCb("DXVK setup failed, continuing without it.");
      } else {
        if (statusCb)
          statusCb("DXVK installed successfully.");
      }
    }
  } else {
    // Validate user-provided Wine
    if (statusCb)
      statusCb("Validating user Wine installation...");
    if (!std::filesystem::exists(m_config.userWineExecutable)) {
      ...
      if (statusCb)
        statusCb("Error: Wine executable not found.");
      return false;
    }
    ...
    if (!std::filesystem::exists(m_config.userPrefixPath)) {
      ...
      if (statusCb)
        statusCb("Error: Wine prefix not found.");
      return false;
    }
    ...
    if (m_config.dxvkEnabled && !isDxvkInstalled()) {
      ...
      if (statusCb)
        statusCb("Installing DXVK to user prefix...");
      setupDxvk(progress);
    }
  }
  ...
  if (statusCb)
    statusCb("Wine environment setup complete!");
  return true;
}
...
std::filesystem::path
WineManager::getPrefixPath(const std::string &subpath) const {
...
bool WineManager::initializePrefix(StatusCallback statusCb) {
  auto prefixPath = getPrefixPath();
  auto wineExecutable = getWineExecutable();

  spdlog::info("Creating Wine prefix at: {}", prefixPath.string());
  if (statusCb)
    statusCb(QString("Creating Wine prefix at: %1")
                 .arg(QString::fromStdString(prefixPath.string())));

  // Create prefix directory
  std::filesystem::create_directories(prefixPath);

  // For umu-run, we just need to run any Windows command to initialize the
  // prefix The wineboot is handled by umu-run automatically
  QProcess process;
  ...
  if (!process.waitForStarted(10000)) {
    spdlog::error("Failed to start umu-run");
    if (statusCb)
      statusCb("Failed to start umu-run.");
    return false;
  }

  // Stream output line-by-line to the status callback
  QString errorOutput;
  while (process.state() != QProcess::NotRunning) {
    if (process.waitForReadyRead(500)) {
      while (process.canReadLine()) {
        ...
        if (!line.isEmpty()) {
          spdlog::debug("umu-run: {}", line.toStdString());
          errorOutput += line + "\n";
          if (statusCb) {
            statusCb(line);
          }
  ...
  if (!remaining.isEmpty()) {
    for (const auto &line :
         QString::fromUtf8(remaining).split('\n', Qt::SkipEmptyParts)) {
      QString trimmed = line.trimmed();
      if (!trimmed.isEmpty()) {
        spdlog::debug("umu-run: {}", trimmed.toStdString());
        errorOutput += trimmed + "\n";
        if (statusCb) {
          statusCb(trimmed);
        }
  ...
  if (process.exitCode() != 0) {
    ...
    if (statusCb)
      statusCb("Prefix initialization failed.");
    return false;
  }
...
bool WineManager::setupDxvk(DownloadProgressCallback progress) {
  ...
  // DXVK DLLs
  std::vector<std::string> dxvkDlls = {"d3d9.dll", "d3d10core.dll", "d3d11.dll",
                                       "dxgi.dll"};
...
QStringList WineManager::buildWineArgs(const std::filesystem::path &executable,
...
QStringList
WineManager::buildWineArgsForConsoleApp(const std::filesystem::path &executable,
                                        const QStringList &args) const {
...
bool WineManager::downloadFile(const std::string &url,
...
bool WineManager::extractArchive(const std::filesystem::path &archive,
                                 const std::filesystem::path &destination) {
  ...
  if (ext == ".xz" || archive.string().find(".tar.xz") != std::string::npos) {
    args << "-xJf" << QString::fromStdString(archive.string()) << "-C"
         << QString::fromStdString(destination.string());
  } else if (ext == ".gz" ||
             archive.string().find(".tar.gz") != std::string::npos) {
    args << "-xzf" << QString::fromStdString(archive.string()) << "-C"
         << QString::fromStdString(destination.string());
  } else {
    // Default to auto-detect
    args << "-xf" << QString::fromStdString(archive.string()) << "-C"
         << QString::fromStdString(destination.string());
  }
